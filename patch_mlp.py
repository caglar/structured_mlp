...
class PatchBasedMLP(MLP):
    ...
    def train(self,
        ...
        if self.output == 1:
            ...
            pre_train_probs =\
            numpy.ones((data.shape[0], self.n_out))
        else:
            pre_train_probs =\
        ...
        # defined in `updates`
        train_model = theano.function(inputs=[index, pindex], outputs=[self.cost, p_y_given_x],
                updates=self.updates,
                givens={
                    self.input: train_set_patches[index * batch_size:(index + 1) * batch_size, pindex],
                    self.y: train_set_pre[index * batch_size:(index + 1) * batch_size, pindex]
                    }
                )
